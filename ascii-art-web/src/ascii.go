...
func GenerateAscii(s string, banner string) string {
	chars := LoadChars(banner) //Load ascii characters from given filename
...
func GetFileInput(filename string) []string {
	...
	if flag.Lookup("test.v") == nil { //If a "test.v" flag is present, the program was called via "go test" of "*_test.go", which can't be in the same folder as "main.go", and thus needs a different path to get to "standard.txt"
		readFile, err = os.Open("src/" + filename + ".txt")
	} else {
...
func SplitByNewline(s string) []string {
	...
	for _, line := range lines {
		output = append(output, strings.Split(line, string([]byte{13, 10}))...) //Works with "go test" calls but not cmd and bash calls
	}
	return output
}
